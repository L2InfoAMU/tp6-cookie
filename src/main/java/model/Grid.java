...
public class Grid implements Iterable<Cell> {
    ...
    private int countBlueNeighbours(Collection<? extends Cell> neighbours) {
        return countNeighbours(neighbours, Cell::isBlueAlive);
    }

    private int countRedNeighbours(Collection<? extends Cell> neighbours) {
        return countNeighbours(neighbours, Cell::isRedAlive);
    }
    ...
    private CellState[][] calculateNextStates() {
        ...
        for (int y = 0; y < nextCellState.length ; y++)
            for (int x = 0; x < nextCellState[y].length; x++)
    ...
    private void updateStates(CellState[][] nextState) {
        for (int y = 0; y < nextState.length ; y++)
            for (int x = 0; x < nextState[y].length; x++)
                getCell(y, x).setState(nextState[y][x]);
    }
    ...
    void clear() {
        ...
        for (CellState[] aNextCellState : nextCellState) {
            Arrays.fill(aNextCellState, CellState.DEAD);
        }
